...
public class RicercaDAO {
		...
		public RicercaDAO()
		{
			query = "SELECT Titolo, Prezzo, ISBN, CopieDisponibili, "
					...
					+ "ID_Casa_Editrice INNER JOIN Autori ON Autori_Codice_Autore = Codice_Autore where Titolo = 1984";
					
		}
		
		public int getNumeroLibri(String Dati)
		{
			query = "SELECT COUNT(*) from Libri where Titolo ='" + Dati+"'";
			return Integer.parseInt((DbConnection.getInstance().eseguiQuery(query).get(0)[0]));
		}

		public Vector<Libro> getLibri(String Dati) {
			...
			query = "SELECT Titolo, Prezzo, ISBN, CopieDisponibili, "
					...
					+ "Generi.Nome from Libri INNER JOIN Generi ON Generi_Codice_Genere"
					+ " = Codice_Genere INNER JOIN Case_Editrici ON Case_Editrici_ID_Casa_Editrice = "
					+ "ID_Casa_Editrice INNER JOIN Autori ON Autori_Codice_Autore = Codice_Autore where Titolo = '"+ Dati+ "'";
